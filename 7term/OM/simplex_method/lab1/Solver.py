...
class Solver:
    ...
    def matrix_C_determinant_is_null(self, vector_c_k_with_wave, matrix_C_inverse, k):
        row_count, col_count = vector_c_k_with_wave.shape
        e_k = zeros(1, row_count)
        e_k[0, k] = 1
        return e_k * matrix_C_inverse * vector_c_k_with_wave == 0

    def get_matrix_B_with_wave(self, vector_c_k_with_wave, matrix_B, k):
        if self.matrix_C_determinant_is_null(vector_c_k_with_wave, matrix_B, k):
    ...
    def get_inverse_matrix(self, matrix_C):
