...
sync_opers:
	...
	SUB CX,DX
	ADD CX,127	;получаем порядок результата
	XOR edx,edx
	PUSH ecx	;проверяем денормализацию
	MOV ECX,24
div_cycle:	;непосредственное деление
	SHL edx,1
	SUB eax,ebx
	TEST eax,eax
	JS divisior_greater
	inc edx
	JMP divisior_less
divisior_greater:	;делитель больше
	add eax,ebx
divisior_less:	;делитель меньше
	SHR ebx,1
loop div_cycle
	POP ecx
	MOV eax,edx
	AND edx,00000000100000000000000000000000b	;проверяем на денормализацию вправо
	CMP edx,0
	JNE normal_result_right
	dec ECX
normal_result_right:
