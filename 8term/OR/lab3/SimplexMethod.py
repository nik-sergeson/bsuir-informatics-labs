...
class SimplexMethod:
    ...
    def find_optimum_plan(self, plan, basis_indexes_set, matrix_A=None, matrix_c=None):
        """
        ...
        if not plan:
            plan = zeros(n, 1)
            basis_plan = basis_inverse_matrix * self.matrix_b
            for i, j in enumerate(basis_indexes_set):
                plan[j, 0] = basis_plan[i, 0]
        while True:
            potentials_vector = get_basis_cost_vector(matrix_c, basis_indexes_set).transpose() * basis_inverse_matrix
            estimates_vestor = zeros(len(not_basis_index_set), 1)
            j_0 = -1
            for i, j in enumerate(not_basis_index_set):
                estimates_vestor[i, 0] = matrix_c[j, 0] - (potentials_vector * matrix_A[:, j])[0, 0]
                if estimates_vestor[i, 0] > self.eps:
                    j_0 = j
                    break
            else:
                return plan, basis_indexes_set
            ...
            i_0, theta_0 = SimplexMethod.get_theta_0(basis_indexes_set, vector_z, plan, self.eps)
            basis_indexes_set.remove(i_0)
            bisect.insort_left(basis_indexes_set, j_0)
            not_basis_index_set.remove(j_0)
            bisect.insort_left(not_basis_index_set, i_0)
            basis_inverse_matrix = get_basis_matrix(matrix_A, basis_indexes_set).inv()
            basis_plan = basis_inverse_matrix * self.matrix_b
            plan = zeros(n, 1)
            for i, j in enumerate(basis_indexes_set):
    ...
    def get_basis_plan(self):
        ...
        while True:
            ...
            not_basis_set = [j for j in j_indexes_set if j not in basis_indexes_set]
            j_0 = SimplexMethod.find_index_with_non_zero_alpha(not_basis_set, basis_indexes_set.index(j_k),
            ...
            if j_0 != -1:
                ...
                matrix_A.col_del(j_k)
                matrix_A.row_del(i_0)
    ...
    def solve(self, maximize):
        self._matrix_A, self.matrix_c = get_cannonical_form(self.matrix_A, self.condition_operators, self.matrix_c,
                                                            maximize)
